...
async function renderAndSaveImage(id, debug = false) {
  // 1) fetch data
  const presetJson = await getFileContent(`presets/${id}.json`);
  ...
  const altRelics = (preset.relics.alternativeRelics || []).filter(s => s && (s.label || s.image));

  // count how many primary relic slots are actually filled
  const filledPrimaryCount = (preset.relics.primaryRelics || [])
    .filter(s => s && (s.label || s.image)).length;
  ...
  const altFam = (preset.familiars.alternativeFamiliars || []).filter(s => s && (s.label || s.image));
  const famAltBottom = altFam.length
    ? (famPrimBottom + 30 + gutter
       - ((preset.familiars.primaryFamiliars || []).filter(s => s && (s.label || s.image)).length) * (familiars.h + familiars.gy)
  ...
  const invTasks = queueSection(
    preset.inventorySlots,
    inventory.cols, inventory.w, inventory.h,
  ...
  const eqTasks = queueSection(
    preset.equipmentSlots,
    equipment.cols, equipment.w, equipment.h,
  ...
  const filledPrimaryRelics = (preset.relics.primaryRelics || []).filter(isFilled);
  ...
  if (altRelics.length) {
    const totalPrimarySlots = (preset.relics.primaryRelics || []).length;
    const emptySlots = totalPrimarySlots - filledPrimaryCount;
    const originalOffset = 37;
    const liftAmount = emptySlots * (relics.h + relics.gy);
    const altY = relicsPrimBottom + originalOffset - liftAmount;
  ...
  const filledPrimaryFamiliars = (preset.familiars.primaryFamiliars || []).filter(isFilled);
  ...
  if (altFam && altFam.length) {
    const originalOffset = 37;
    const emptyCount = (preset.familiars.primaryFamiliars || []).length - filledPrimaryFamiliars.length;
    const liftFam = emptyCount * (familiars.h + familiars.gy);
  ...
  // 12) space for visualing testing/debugging
  // if (debug) {  }
